...
export const createUsuario = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('CALL VerificarUsuarioPorUsername(?)', [username]);
    const existingUser = rows[0]; // Asegúrate de acceder al primer conjunto de resultados
...
export const loginUsuario = async (req, res) => {
  try {
    ...
    // Llamar al procedimiento almacenado para verificar usuario
    const [rows] = await pool.query("CALL VerificarUsuarioPorUsername(?)", [username]);
    ...
    const usuario = rows[0][0]; // Tomamos el primer usuario encontrado
    ...
    // Datos del usuario
    const datosUsuario = { idUsuarios: usuario.idUsuarios, username, rol: usuario.idRol };
    ...
    await pool.query(
      "CALL InsertarLogin(?, ?, ?, ?)",
      [token, fecha, hora, usuario.idUsuarios]
    );
...
export const verifyToken = async (req, res) => {
  try {
    ...
    jwt.verify(token, TOKEN_SECRET, async (error, user) => {
      ...
      const [userFound] = await pool.query("CALL VerificarUsuarioPorUsername(?)", [user.username]);
...
export const updateUsuario = async (req, res) => {
  try {
    ...
    const [userExists] = await pool.query("CALL ObtenerLoginPorID(?)", [id]);
    ...
    const [updatedUser] = await pool.query("CALL ObtenerUsuarioPorID(?)", [id]);
